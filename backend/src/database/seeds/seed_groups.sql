/**
 * Comprehensive seed data for Groups System Testing
...
INSERT INTO users (username, email, password, role, bio, avatar_url, location_lat, location_lon, location_name, location_country)
VALUES (
  'admin_alice',
  'alice@groups.test',
  '$2a$10$dOYhBGMXdvRGwOuz501EzOvmM5G7ZOBpy6b.j5z.zewBATdaRHSX.', -- password: "test123"
  'user',
...
INSERT INTO group_memberships (group_id, user_id, role, status)
VALUES
  ((SELECT id FROM groups WHERE slug = 'techcommunity'), (SELECT id FROM users WHERE username = 'charlie_dev'), 'member', 'active'),
...
VALUES
  ...
  ((SELECT id FROM groups WHERE slug = 'gaminghub'), (SELECT id FROM users WHERE username = 'charlie_dev'), 'member', 'active'),
...
INSERT INTO group_posts (group_id, author_id, title, content, content_type, is_pinned)
VALUES (
  (SELECT id FROM groups WHERE slug = 'techcommunity'),
  (SELECT id FROM users WHERE username = 'admin_alice'),
...
INSERT INTO group_posts (group_id, author_id, title, content, content_type)
VALUES (
  (SELECT id FROM groups WHERE slug = 'techcommunity'),
...
DO $$
DECLARE
  tech_group_id INT := (SELECT id FROM groups WHERE slug = 'techcommunity');
  charlie_post_id INT;
  ...
  charlie_id INT := (SELECT id FROM users WHERE username = 'charlie_dev');
  comment1_id INT;
  comment2_id INT;
BEGIN
  -- Get Charlie's post
  SELECT id INTO charlie_post_id FROM group_posts
  WHERE group_id = tech_group_id
  AND author_id = charlie_id
  AND title LIKE '%full-stack app%';

  IF charlie_post_id IS NOT NULL THEN
    -- Alice's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (tech_group_id, charlie_post_id, alice_id, 'This is amazing! Congrats on shipping your first app. How did you handle WebSocket reconnection logic?')
    RETURNING id INTO comment1_id;

    -- Charlie's reply to Alice
    INSERT INTO group_comments (group_id, post_id, author_id, parent_id, content)
    VALUES (tech_group_id, charlie_post_id, charlie_id, comment1_id, 'Thanks! I used socket.io''s built-in reconnection with exponential backoff. Also implemented a heartbeat to detect stale connections. Took a while to get right but works great now!');

    -- Diana's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (tech_group_id, charlie_post_id, diana_id, 'Love seeing the tech stack! Did you design the UI yourself or use a component library?')
    RETURNING id INTO comment2_id;

    -- Charlie's reply to Diana
    INSERT INTO group_comments (group_id, post_id, author_id, parent_id, content)
    VALUES (tech_group_id, charlie_post_id, charlie_id, comment2_id, 'I used styled-components for custom components. Wanted to learn it properly instead of relying on Material-UI. It was challenging but learned so much!');

    -- Diana's follow-up reply
    INSERT INTO group_comments (group_id, post_id, author_id, parent_id, content)
    VALUES (tech_group_id, charlie_post_id, diana_id, comment2_id, 'Good choice! styled-components is powerful once you get the hang of it. Would love to see screenshots if you''re comfortable sharing!');

    -- Bob's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (tech_group_id, charlie_post_id, bob_id, 'Impressive work! Any plans to open source it?');
  END IF;
END $$;

-- Comments on Diana's form design post (Tech Community)
DO $$
DECLARE
  tech_group_id INT := (SELECT id FROM groups WHERE slug = 'techcommunity');
  diana_post_id INT;
  ...
  comment1_id INT;
BEGIN
  SELECT id INTO diana_post_id FROM group_posts
  WHERE group_id = tech_group_id
  AND author_id = diana_id
  AND title LIKE '%form%';

  IF diana_post_id IS NOT NULL THEN
    -- Charlie's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (tech_group_id, diana_post_id, charlie_id, 'Great tips! I always struggle with inline validation. Do you validate on every keystroke or on blur?')
    RETURNING id INTO comment1_id;

    -- Diana's reply
    INSERT INTO group_comments (group_id, post_id, author_id, parent_id, content)
    VALUES (tech_group_id, diana_post_id, diana_id, comment1_id, 'Good question! I usually do it on blur first, then on every keystroke once they''ve touched the field. This way you don''t show errors while they''re still typing.');

    -- Bob's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (tech_group_id, diana_post_id, bob_id, 'The "mobile-first" point is so important. I''ve seen too many forms that are impossible to use on phones. Nice writeup!');

    -- Jack's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (tech_group_id, diana_post_id, jack_id, 'As someone who fills out a lot of forms, THANK YOU for spreading this knowledge! 😄');
  END IF;
END $$;

-- Comments on Grace's carbonara recipe (Foodie Heaven)
DO $$
DECLARE
  food_group_id INT := (SELECT id FROM groups WHERE slug = 'foodieheaven');
  grace_post_id INT;
  ...
  comment2_id INT;
BEGIN
  SELECT id INTO grace_post_id FROM group_posts
  WHERE group_id = food_group_id
  AND author_id = grace_id
  AND title LIKE '%carbonara%';

  IF grace_post_id IS NOT NULL THEN
    -- Frank's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (food_group_id, grace_post_id, frank_id, 'This recipe is GOLD! Made it last night and it was incredible. The pasta water trick really does make all the difference.')
    RETURNING id INTO comment1_id;

    -- Grace's reply
    INSERT INTO group_comments (group_id, post_id, author_id, parent_id, content)
    VALUES (food_group_id, grace_post_id, grace_id, comment1_id, 'So happy you loved it! My nonna would be proud 🥰');

    -- Evan's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (food_group_id, grace_post_id, evan_id, 'Quick question: can I substitute guanciale with pancetta if I can''t find it?')
    RETURNING id INTO comment2_id;

    -- Grace's reply about guanciale
    INSERT INTO group_comments (group_id, post_id, author_id, parent_id, content)
    VALUES (food_group_id, grace_post_id, grace_id, comment2_id, 'You can, but the flavor won''t be quite the same. Guanciale is fattier and has more depth. If you must substitute, pancetta is better than bacon at least!');

    -- Jack's follow-up
    INSERT INTO group_comments (group_id, post_id, author_id, parent_id, content)
    VALUES (food_group_id, grace_post_id, jack_id, comment2_id, 'I found guanciale at an Italian deli in my city. Worth searching for specialty stores!');

    -- Jack's main comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (food_group_id, grace_post_id, jack_id, 'Trying this tonight! Been craving authentic carbonara forever.');
  END IF;
END $$;

-- Comments on Iris's D&D post (Gaming Hub)
DO $$
DECLARE
  gaming_group_id INT := (SELECT id FROM groups WHERE slug = 'gaminghub');
  iris_post_id INT;
  ...
  comment1_id INT;
BEGIN
  SELECT id INTO iris_post_id FROM group_posts
  WHERE group_id = gaming_group_id
  AND author_id = iris_id
  AND title LIKE '%D&D%';

  IF iris_post_id IS NOT NULL THEN
    -- Henry's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (gaming_group_id, iris_post_id, henry_id, 'I''m interested! Played a few sessions before. Would love to play a dwarf cleric. Are you okay with new-ish players?')
    RETURNING id INTO comment1_id;

    -- Iris's reply
    INSERT INTO group_comments (group_id, post_id, author_id, parent_id, content)
    VALUES (gaming_group_id, iris_post_id, iris_id, comment1_id, 'Absolutely! New players are very welcome. Dwarf cleric sounds great - we could use a healer. I''ll DM you the Discord link!');

    -- Charlie's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (gaming_group_id, iris_post_id, charlie_id, 'Never played but always wanted to try! Is it cool if I join as a complete beginner?');

    -- Iris's reply to Charlie
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (gaming_group_id, iris_post_id, iris_id, NULL, '@charlie_dev Yes! I love teaching new players. I''ll help you create a character and learn the basics. What kind of character interests you? Fighter is great for beginners.');

    -- Jack's comment
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (gaming_group_id, iris_post_id, jack_id, 'Saturdays work for me! I''d like to play a rogue if that''s open. Been playing for 3 years.');
  END IF;
END $$;

-- Comments on Henry's tournament post (Gaming Hub)
DO $$
DECLARE
  gaming_group_id INT := (SELECT id FROM groups WHERE slug = 'gaminghub');
  ...
  jack_id INT := (SELECT id FROM users WHERE username = 'jack_social');
BEGIN
  SELECT id INTO tournament_post_id FROM group_posts
  WHERE group_id = gaming_group_id
  AND author_id = henry_id
  AND title LIKE '%Tournament%';

  IF tournament_post_id IS NOT NULL THEN
    -- Vote comments
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES
      (gaming_group_id, tournament_post_id, charlie_id, '1. Valorant\n2. Gold 2\n3. Y - need a team!'),
      (gaming_group_id, tournament_post_id, iris_id, '1. League of Legends\n2. Platinum 3\n3. N - already have a team'),
      (gaming_group_id, tournament_post_id, jack_id, '1. CS2\n2. DMG\n3. Y - can flex any role');

    -- Henry's response
    INSERT INTO group_comments (group_id, post_id, author_id, content)
    VALUES (gaming_group_id, tournament_post_id, henry_id, 'Great responses! Looking like Valorant is winning. I''ll set up a team formation thread soon for those looking for teammates.');
  END IF;
END $$;
...
SET post_count = (
  SELECT COUNT(*)
  FROM group_posts gp
  WHERE gp.group_id = g.id AND gp.status = 'published'
);
